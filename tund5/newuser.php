<?php
  ...
  if(!empty($_POST["birthDay"]) and !empty($_POST["birthMonth"]) and !empty($_POST["birthYear"])){
	  ...
	  #checkdate ootab kome täisarvu: kuu, päev, aasta
   ...
   if(empty($firstNameError) and empty($firstNameError) and empty($birthMonthError) and empty($birthYearError) and empty($birthDayError) and empty($birthDateError) and empty($genderError)and empty($emailError)and empty($passwordError)) {
   $notice = singup($firstName, $lastName, $birthDate, $gender, $_POST["email"], $_POST["password"]);
